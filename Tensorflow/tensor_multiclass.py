...
class MulticlassHyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        model.add(layers.Dense(len(set(train_labels))))
...
def build_model(input_shape, input_train, target_train, configuration_layer_tuner, flatten=False):
    ...
    hypermodel = MulticlassHyperModel(input_shape_converted, input_shape, configuration_layer_tuner, flatten=flatten)
...
model = build_model(flatten=True, input_shape=(28, 28),
                    input_train=train_images, target_train=train_labels,
